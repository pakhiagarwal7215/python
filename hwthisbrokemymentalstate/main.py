...
def apply_colour_filter(image,filter_type,intensity=50):
...
def interactive_color_filter(image_path):
    ...
    while True:
        ...
        if filter_type == "r":
            filtered_image=apply_colour_filter(image,"red_tint")

        elif filter_type == "b":
            filtered_image = apply_colour_filter(image,"blue_tint")

        elif filter_type == "g":
            filtered_image = apply_colour_filter(image,"green_tint")  

        elif filter_type == "i":
            filtered_image = apply_colour_filter(image,"increase_red",intensity=50)

        elif filter_type == "d":
            filtered_image = apply_colour_filter(image,"decrease_blue",intensity=50)

        elif filter_type == "up_arrow":
            filtered_image = apply_colour_filter(image,"increase_green",intensity=50) 
            
        elif filter_type == "down_arrow":
            filtered_image = apply_colour_filter(image,"decrease_red",intensity=50)
...
interactive_color_filter('merightnow.jpg')